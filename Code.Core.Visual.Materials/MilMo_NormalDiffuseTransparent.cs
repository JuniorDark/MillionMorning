...
public class MilMo_NormalDiffuseTransparent : MilMo_Material
{
	private const string SHADER_FILENAME = "Junebug/DiffuseTrans";
	...
	protected override string Name => "Junebug/DiffuseTrans";

	public MilMo_NormalDiffuseTransparent()
		: base("Junebug/DiffuseTrans")
	{
	}
	...
	public static void Write(Material material, MilMo_SFFile file, int index, MilMo_Material template)
	{
		try
		{
			MilMo_NormalDiffuseTransparent milMo_NormalDiffuseTransparent = template as MilMo_NormalDiffuseTransparent;
			file.AddRow();
			file.Write("<MAT>");
			if (index != -1)
			{
			...
			{
				MilMo_Material.WriteTexture(material, file, milMo_NormalDiffuseTransparent, "_MainTex", "Diffuse");
			}
			...
			if ((bool)texture && texture.name != "VelvetRamp")
			{
				...
				if (milMo_NormalDiffuseTransparent == null || text != milMo_NormalDiffuseTransparent._velvetRamp)
				{
					file.AddRow();
			...
			if (milMo_NormalDiffuseTransparent == null || !MilMo_Utility.Equals(color, milMo_NormalDiffuseTransparent.MainColor))
			{
				file.AddRow();
			...
			if (milMo_NormalDiffuseTransparent == null || !MilMo_Utility.Equals(color2, milMo_NormalDiffuseTransparent._velvetColor))
			{
				file.AddRow();
			...
			if (milMo_NormalDiffuseTransparent == null || !MilMo_Utility.Equals(vector, milMo_NormalDiffuseTransparent._velvetChannel))
			{
				file.AddRow();
			...
			if (!MilMo_Utility.Equals(vector2, new Vector2(0f, 0f)) && (milMo_NormalDiffuseTransparent == null || !MilMo_Utility.Equals(vector2, milMo_NormalDiffuseTransparent._uvScroll)))
			{
				file.AddRow();
				file.Write("UVScroll");
				file.Write(vector2);
			...
			if (milMo_NormalDiffuseTransparent == null || (double)Math.Abs(@float - milMo_NormalDiffuseTransparent._alpha) > 0.001)
			{
				file.AddRow();
				file.Write("Alpha");
				file.Write(@float);
