...
public class MilMo_FurnitureDiffuse : MilMo_Material
{
	private const string SHADER_FILENAME = "Junebug/FurnitureDiffuse";
	...
	protected override string Name => "Junebug/FurnitureDiffuse";

	public MilMo_FurnitureDiffuse()
		: base("Junebug/FurnitureDiffuse")
	{
	}
	...
	public static void Write(Material material, MilMo_SFFile file, int index, MilMo_Material template)
	{
		try
		{
			MilMo_FurnitureDiffuse milMo_FurnitureDiffuse = template as MilMo_FurnitureDiffuse;
			file.AddRow();
			file.Write("<MAT>");
			if (index != -1)
			{
			...
			{
				MilMo_Material.WriteTexture(material, file, milMo_FurnitureDiffuse, "_MainTex", "Diffuse");
			}
			...
			if ((bool)texture && texture.name != "VelvetRamp")
			{
				...
				if (milMo_FurnitureDiffuse == null || text != milMo_FurnitureDiffuse._velvetRamp)
				{
					file.AddRow();
			...
			if (milMo_FurnitureDiffuse == null || !MilMo_Utility.Equals(color, milMo_FurnitureDiffuse.MainColor))
			{
				file.AddRow();
			...
			if (milMo_FurnitureDiffuse == null || !MilMo_Utility.Equals(color2, milMo_FurnitureDiffuse._velvetColor))
			{
				file.AddRow();
			...
			if (milMo_FurnitureDiffuse == null || !MilMo_Utility.Equals(vector, milMo_FurnitureDiffuse._velvetChannel))
			{
				file.AddRow();
