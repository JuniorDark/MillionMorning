...
public class MilMo_DiffuseAmbient : MilMo_Material
{
	private const string SHADER_FILENAME = "Junebug/DiffuseAmbient";
	...
	protected override string Name => "Junebug/DiffuseAmbient";

	public MilMo_DiffuseAmbient()
		: base("Junebug/DiffuseAmbient")
	{
	}
	...
	public static void Write(Material material, MilMo_SFFile file, int index, MilMo_Material template)
	{
		try
		{
			MilMo_DiffuseAmbient milMo_DiffuseAmbient = template as MilMo_DiffuseAmbient;
			file.AddRow();
			file.Write("<MAT>");
			if (index != -1)
			{
			...
			{
				MilMo_Material.WriteTexture(material, file, milMo_DiffuseAmbient, "_MainTex", "Diffuse");
			}
			if ((bool)material.GetTexture(AmbientTex))
			{
				MilMo_Material.WriteTexture(material, file, milMo_DiffuseAmbient, "_AmbientTex", "Ambient");
			}
			...
			if ((bool)texture && texture.name != "VelvetRamp")
			{
				...
				if (milMo_DiffuseAmbient == null || text != milMo_DiffuseAmbient._velvetRamp)
				{
					file.AddRow();
			...
			if (milMo_DiffuseAmbient == null || !MilMo_Utility.Equals(color, milMo_DiffuseAmbient.MainColor))
			{
				file.AddRow();
			...
			if (milMo_DiffuseAmbient == null || !MilMo_Utility.Equals(color2, milMo_DiffuseAmbient._ambientColor))
			{
				file.AddRow();
			...
			if (milMo_DiffuseAmbient == null || !MilMo_Utility.Equals(color3, milMo_DiffuseAmbient._velvetColor))
			{
				file.AddRow();
			...
			if (milMo_DiffuseAmbient == null || !MilMo_Utility.Equals(vector, milMo_DiffuseAmbient._velvetChannel))
			{
				file.AddRow();
