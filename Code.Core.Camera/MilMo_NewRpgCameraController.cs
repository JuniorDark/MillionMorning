...
public sealed class MilMo_NewRpgCameraController : MilMo_GameCameraController
{
	...
	private const float ORBIT_DAMPING = 12f;
	...
	public MilMo_NewRpgCameraController()
	{
	...
	{
		base.HookUp();
		if (MilMo_Input.DefaultMode == MilMo_Input.ControlMode.NewMmorpg)
		{
			MilMo_Input.SetControlMode(MilMo_Input.ControlMode.NewMmorpg);
		}
	...
	protected override void ReadOrbitInput(bool readLookup = true, bool readPan = true)
	{
		if (readLookup)
		{
			...
			float num2 = -4.42f * num * Settings.CameraSensitivity;
			_freeLookLookupModifier += num2;
		}
		if (readPan)
		{
			float axisX = MilMo_CameraController.GetAxisX();
			float num3 = 4.42f * axisX * Settings.CameraSensitivity;
			MilMo_GameCameraController.WantedPan += num3;
		}
	}
	...
	private void ApplyPlayerPosition(Vector3 aTargetCenter)
	{
		...
		MilMo_GameCameraController.Lookup = Mathf.LerpAngle(MilMo_GameCameraController.Lookup, MilMo_GameCameraController.WantedLookup, 12f * Time.deltaTime);
		...
		MilMo_GameCameraController.Pan = MilMo_GameCameraController.WantedPan;
